...
class LoggerDriver: NSObject, SwishhpushLoggerDriverProtocol {
    let logger: OSLog
...
class NotificationService: UNNotificationServiceExtension {
    let oslogger = OSLog(subsystem: "tech.swishh.ios.notif", category: "gomobile")
    let pushStandalone: SwishhpushPushStandalone

    var contentHandler: ((UNNotificationContent) -> Void)?
    var bestAttemptContent: UNMutableNotificationContent?

    override init() {
        let config = SwishhpushNewConfig()!
        ...
        #endif
        pushStandalone = SwishhpushNewPushStandalone(config)!
        super.init()
    }
    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        guard let data = request.content.userInfo[SwishhpushServicePushPayloadKey] as? String else {
        ...
        var decrypted: SwishhpushFormatedPush
        do {
    ...
    //
    // The notificationService app extension doesn't get automatically the app language from UserDefaults until it restarts. So the Swishh app puts the correct language to the Common database (shared between Swishh app and NotificationService) and getLocalizedBundle loads the corresponding bundle from this database.
