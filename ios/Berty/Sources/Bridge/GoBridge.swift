...
class GoBridge: NSObject {
    let logger = LoggerDriver("tech.swishh", "react")

    // protocol
    var bridgeMessenger: SwishhbridgeBridge?
    let rootDir: String
    ...
    override init() {
        // set swishh dir for persistence
    ...
    @objc func initBridge(_ resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        do {
            if self.bridgeMessenger != nil {
                throw NSError(domain: "tech.swishh.gobridge", code: 1, userInfo: [NSLocalizedDescriptionKey : "already started"])
            }

            var err: NSError?
            guard let config = SwishhbridgeNewConfig() else {
                throw NSError(domain: "tech.swishh.gobridge", code: 2, userInfo: [NSLocalizedDescriptionKey : "unable to create config"])
            }

            config.setLoggerDriver(LoggerDriver("tech.swishh", "gomobile"))
            ...
            let bridgeMessenger = SwishhbridgeNewBridge(config, &err)
            if err != nil {
                throw err!
            }
    ...
    @objc func invokeBridgeMethod(_ method: String, b64message: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        do {
            guard let bridgeMessenger = self.bridgeMessenger else {
                throw NSError(domain: "tech.swishh.gobridge", code: 3, userInfo: [NSLocalizedDescriptionKey : "bridgeMessenger isn't started"])
            }
    ...
    @objc func getProtocolAddr(_ resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        do {
          if self.bridgeMessenger == nil {
              throw NSError(domain: "tech.swishh.gobridge", code: 4, userInfo: [NSLocalizedDescriptionKey : "bridgeMessenger isn't started"])
          }
