...
export class MessengerServiceMock implements Partial<IMessengerServiceMock> {
	...
	InstanceShareableSwishhID = async (_request: beapi.messenger.AccountUpdate.IRequest) => {
		return {
	...
	ConversationCreate = async (request: beapi.messenger.ConversationCreate.IRequest) => {
		...
		const link = `https://swishh.tech/mock#group/${pk}/name=${request.displayName}`
		...
		this.emitStreamEvent({
			...
			payload: beapi.messenger.StreamEvent.ConversationUpdated.encode({
				conversation: {
					...
					type: beapi.messenger.Conversation.Type.MultiMemswishhpe,
					isOpen: false,
					link,
					unreadCount: 0,
	...
	ParseDeepLink = async (request: beapi.messenger.ParseDeepLink.IRequest) => {
		...
		const result: beapi.messenger.ISwishhLink = {}
		...
		switch (kindString) {
			case 'group':
				result.kind = beapi.messenger.SwishhLink.Kind.GroupV1Kind
				...
				result.swishhGroup = { displayName, group: { publicKey: groupPK } }
				break
			case 'contact':
				result.kind = beapi.messenger.SwishhLink.Kind.ContactInviteV1Kind
				...
				result.swishhId = { displayName, accountPk: pk }
				break
			default:
				result.kind = beapi.messenger.SwishhLink.Kind.UnknownKind
				break
		}
	...
	ContactRequest = async (request: beapi.messenger.ContactRequest.IRequest) => {
		...
		if (parsedLink.link?.kind !== beapi.messenger.SwishhLink.Kind.ContactInviteV1Kind) {
		...
		this.emitStreamEvent({
			...
			payload: beapi.messenger.StreamEvent.ContactUpdated.encode({
				contact: {
					publicKey: contactPK,
					displayName: parsedLink.link.swishhId?.displayName,
					createdDate: longNow,
					infoDate: longNow,
	...
	ConversationJoin = async (request: beapi.messenger.ConversationJoin.IRequest) => {
		const parsedLink = await this.ParseDeepLink({ link: request.link })
		if (parsedLink.link?.kind !== beapi.messenger.SwishhLink.Kind.GroupV1Kind) {
			throw new Error('invalid link kind')
		}
		const longNow = Long.fromNumber(Date.now())
		const convPK = faker.datatype.uuid()
		...
		this.emitStreamEvent({
			...
			payload: beapi.messenger.StreamEvent.ConversationUpdated.encode({
				conversation: {
					...
					accountMemberPublicKey: memberPK,
					createdDate: longNow,
					displayName: parsedLink.link.swishhGroup?.displayName,
					link: request.link,
				},
			}).finish(),
		})
