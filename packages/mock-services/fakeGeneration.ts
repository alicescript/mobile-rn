...
import { randomValueFromEnum } from '@swishh/utils/testing/mockServices.test'
...
const genFakeAccount = ({
	...
	return {
		...
		link: `https://swishh.tech/mock#contact/${publicKey}/name=${displayName}`,
		publicKey: publicKey,
	}
}
...
const genFakeMultiMemberConversations = ({ count = 20, baseDate = Date.now() }) =>
	range(0, count).map(() => {
		...
		const conv: beapi.messenger.IConversation = {
			...
			isOpen: false,
			link: `https://swishh.tech/mock#group/${publicKey}/name=${displayName}`,
