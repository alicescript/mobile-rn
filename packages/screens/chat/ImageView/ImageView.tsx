...
import { ForwardToSwishhContactModal } from './components/ForwardToSwishhContactModal'
	...
	const MENU_LIST = [
		...
		},
		{
			...
			onPress() {
				...
				Share.open({
					title: img.displayName || img.filename || 'Image from Swishh',
					url: img.uri,
				})
					.then(() => {})
					.catch(err => {
		...
		{
			title: t('chat.files.forward-swishh'),
			onPress() {
