...
import { swishhMethodsHooks, useAppDispatch } from '@swishh/hooks'
import {
...
const AddBotBody: React.FC<{
	...
	const { call: requestContact, done, error } = swishhMethodsHooks.useContactRequest()
	const {
		reply: pdlReply,
		error: pdlError,
		call,
		done: pdlDone,
	} = swishhMethodsHooks.useParseDeepLink()
	...
	const onAdd = () => {
		if (pdlDone && !pdlError && pdlReply && pdlReply.link?.swishhId?.accountPk) {
			dispatch(
				setPersistentOption({
					type: PersistentOptionsKeys.Suggestions,
					payload: {
						...
						[displayName]: {
							...
							pk: base64ToURLBase64(
								Buffer.from(pdlReply.link.swishhId.accountPk).toString('base64'),
							),
						},
					},
				}),
	...
	return pdlReply?.link?.swishhId?.accountPk ? (
		<ActionModal
