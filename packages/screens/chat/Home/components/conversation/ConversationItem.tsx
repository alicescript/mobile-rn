...
import { ConversationAvatar } from '@swishh/components/avatars'
import { useStyles } from '@swishh/contexts/styles'
import { useAppSelector, useOneToOneContact, useLastConvInteraction } from '@swishh/hooks'
import { selectChatInputText } from '@swishh/redux/reducers/chatInputs.reducer'
import { selectChatInputIsSending } from '@swishh/redux/reducers/chatInputsVolatile.reducer'
import { ParsedInteraction } from '@swishh/utils/api'
		...
		<ConversationButton publicKey={publicKey} isAccepted={isAccepted} type={type} isLast={isLast}>
			...
			<View style={[flex.justify.spaceAround, styles.content]}>
				...
				<ConversationDescStatus
					...
					unreadCount={unreadCount ?? undefined}
					isAccepted={isAccepted || type === beapi.messenger.Conversation.Type.MultiMemswishhpe}
				/>
			</View>
		</ConversationButton>
	) : null
