...
import EmptyChat from '@swishh/assets/logo/empty_chat.svg'
import { PrimaryFloatingButton } from '@swishh/components'
import { useLayout } from '@swishh/components/hooks'
import { ButtonSettingV2 } from '@swishh/components/shared-components'
		...
		() =>
			searching
				? pickBy(
						conversationsDict,
						val =>
							val?.type === beapi.messenger.Conversation.Type.MultiMemswishhpe &&
	...
	return (
		<View style={[styleBackground, { flex: 1 }]}>
			...
			>
				...
				{searchText?.length ? (
					<>
						{(searchText.startsWith('https://swishh.tech/id') ||
							searchText.startsWith('swishh://')) && (
							<View style={[{ flexDirection: 'row', justifyContent: 'center' }]}>
								<View style={[border.shadow.large, border.radius.medium]}>
									<ButtonSettingV2
										text='Open Swishh Link'
										icon='external-link-outline'
										accessibilityLabel='Open Swishh Link'
										onPress={() =>
