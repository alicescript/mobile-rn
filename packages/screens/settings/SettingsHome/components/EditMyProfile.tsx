...
import { SecondaryButton, SmallInput } from '@swishh/components'
import { UnifiedText } from '@swishh/components/shared-components/UnifiedText'
import { useStyles } from '@swishh/contexts/styles'
import { useAccount, useMessengerClient, useThemeColor, useUpdateAccount } from '@swishh/hooks'
...
export const EditMyProfile: React.FC<EditProfileProps> = ({ hide }) => {
	...
	const handleSave = async () => {
		try {
			...
			if (updated) {
				// update account in swishhmessenger
				await client?.accountUpdate(update)
				// update account in swishhaccount
				await updateAccount({
