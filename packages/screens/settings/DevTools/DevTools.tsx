...
import { DropDownPicker, Item } from '@swishh/components/shared-components/DropDownPicker'
import {
	ButtonSetting,
	ButtonSettingItem,
	ButtonSettingRow,
...
import { createServiceClient } from '@swishh/grpc-bridge'
import * as middleware from '@swishh/grpc-bridge/middleware'
import { bridge as rpcBridge } from '@swishh/grpc-bridge/rpc'
import {
	useAllConversations,
	useAllInteractions,
	useAppDispatch,
...
import { languages } from '@swishh/i18n/locale/languages'
import { GoBridge } from '@swishh/native-modules/GoBridge'
...
import { storageGet, storageSet } from '@swishh/utils/accounts/accountClient'
import { exportLogfile } from '@swishh/utils/accounts/accountUtils'
...
import { showNeedRestartNotification } from '@swishh/utils/notification/notif-in-app'
...
const DiscordShareButton: React.FC = () => {
	...
	const { call, done, error } = swishhMethodsHooks.useDevShareInstanceSwishhID()
	const { t } = useTranslation()
	const colors = useThemeColor()
	...
	return (
		<>
			<ButtonSetting
				...
				iconColor={colors['alt-secondary-background-header']}
				onPress={() => {
					new Player('Notif_Swishh02.mp3', { mixWithOthers: true }).play()
				}}
			/>
			<ButtonSetting
