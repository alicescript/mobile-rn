...
import { HorizontalDuo, SecondaryButtonIconLeft, TertiaryButtonIconLeft } from '@swishh/components'
...
import { InteractionGroupInvitation } from '@swishh/utils/api'
	...
	React.useEffect(() => {
		if (client && !convPk && link && !conv) {
			...
			client
				...
				.then(reply => {
					setPdlInfo({
						displayName: reply.link?.swishhGroup?.displayName || '',
						convPk: base64ToURLBase64(
							Buffer.from(reply.link?.swishhGroup?.group?.publicKey || '').toString('base64'),
						),
					})
				})
				.catch(err => {
