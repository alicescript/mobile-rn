...
import { WelshMessengerServiceClient } from '@swishh/grpc-bridge/welsh-clients.gen'
import { useMessengerClient, useThemeColor } from '@swishh/hooks'
import { useNavigation } from '@swishh/navigation'
import { InteractionUserMessage, ParsedInteraction } from '@swishh/utils/api'
import { pbDateToNum, timeFormat } from '@swishh/utils/convert/time'
...
export async function isSwishhDeepLink(
	...
	return (
		...
		>
			{message ? (
				<Hyperlink
					onPress={async url => {
						if (client && (await isSwishhDeepLink(client, url))) {
