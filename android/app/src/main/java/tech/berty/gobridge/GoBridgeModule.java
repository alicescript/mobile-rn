...
import tech.swishh.android.BuildConfig;
import tech.swishh.rootdir.RootDirModule;

// go packages
import swishhbridge.PromiseBlock;
import swishhbridge.Swishhbridge;
import swishhbridge.Bridge;
import swishhbridge.Config;
import tech.swishh.gobridge.bledriver.BleInterface;

public class GoBridgeModule extends ReactContextBaseJavaModule {
  private final static String TAG = "GoBridge";
  private final ReactApplicationContext reactContext;
  private final static LoggerDriver rnlogger = new LoggerDriver("tech.swishh", "react");
  ...
  public GoBridgeModule(ReactApplicationContext reactContext) {
    ...
    tempDir = new File(reactContext.getCacheDir().getAbsolutePath() + "/swishh");
  }
 ...
 // Protocol //
  ...
  public void initBridge(Promise promise) {
    try {
      ...
      final Config config = Swishhbridge.newConfig();
      if (config == null) {
        throw new Exception("");
      }

      // init logger
      LoggerDriver logger = new LoggerDriver("tech.swishh", "protocol");
      ...
      // set NearBy driver
      // SwishhNearbyDriver NBDriver = new SwishhNearbyDriver(reactContext);
      // config.setNBDriver(NBDriver);

      // set native keystore driver
      config.setKeystoreDriver(this.keystoreDriver);

      this.bridgeMessenger = Swishhbridge.newBridge(config);
      promise.resolve(true);
    } catch (Exception err) {
      promise.reject(err);
    }
