package tech.swishh.gobridge.nearbydriver.base;
...
import tech.swishh.gobridge.nearbydriver.lifecycle.UserAcceptCallback;
import tech.swishh.gobridge.nearbydriver.lifecycle.UserConnectionCallback;
import tech.swishh.gobridge.nearbydriver.lifecycle.UserMessageCallback;
import tech.swishh.gobridge.nearbydriver.lifecycle.UserRequestCallback;
import tech.swishh.gobridge.nearbydriver.lifecycle.UserSearchCallback;
import tech.swishh.gobridge.nearbydriver.model.Endpoint;
import tech.swishh.gobridge.nearbydriver.util.SwishhLogger;
...
public class NearbyDriver {
    ...
    private NearbyDriver(Context context) {
        SwishhLogger.d(TAG, "new NDBDriver instance");
        if (mNBDriver != null) {
    ...
    public static synchronized NearbyDriver getInstance(Context appContext) {
        SwishhLogger.d(TAG, "getInstance called");
        if (mNBDriver == null) {
            ...
            public void onEndpointLost(@NonNull String endpointId) {
                ...
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        SwishhLogger.d("SwishhSDK", "Searching started");
                    }
                }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                SwishhLogger.d("SwishhSDK", "Searching failed: " + e.toString());
            }
        });
    }
            ...
            public void onDisconnected(@NonNull String endpointId) {
               handleOnDisconnected(userConnectionCallback, endpointId);
                ...
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        SwishhLogger.d("SwishhSDK", "Sharing success");
                    }
                })
                .addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        SwishhLogger.d("SwishhSDK", "Sharing failed: "  + e.toString());
                    }
                });
    }

    private void handleRejectedConnection(String endpointId, UserConnectionCallback userConnectionCallback) {
        SwishhLogger.d("SwishhSDK", "Failed to connect to " + endpointId);
        ...
        SwishhLogger.d("SwishhSDK", "Rejected endpoint removed " + endpointId);
    ...
    private void handleConnectedToEndpoint(String endpointId, UserConnectionCallback userConnectionCallback) {
        ...
        SwishhLogger.d("SwishhSDK", "Connected to " + endpointId);
...
//                userConnectionCallback.onConnectionRequested(connectionInfo.getEndpointName(), endpointId, connectionInfo.isIncomingConnection());
            ...
            @Override
            public void onSuccess(Void aVoid) {
                SwishhLogger.d("SwishhSDK", "Connection Requested0");
                userRequestCallback.onConnectionRequested(true, userName, userId, 0);
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                SwishhLogger.d("SwishhSDK", "Fail to request 0 " + e.toString());
        ...
        mConnectionsClient.acceptConnection(userId, new PayloadCallback() {
            ...
            public void onPayloadReceived(@NonNull String s, @NonNull Payload payload) {
                if (payload.getType() == Payload.Type.FILE) {
                    ...
                    SwishhLogger.d("SwishhSDK", "Payload File incoming ");
                } else {
                    try {
                        byte[] message = payload.asBytes();
                        userMessageCallback.onMessageReceived(s, message);
                    } catch (Exception e) {
                        SwishhLogger.e("SwishhSDK", "Payload Received Error " + e.toString());
                    }
                }
            }
            ...
            public void onPayloadTransferUpdate(@NonNull String s, @NonNull PayloadTransferUpdate payloadTransferUpdate) {
                if (payloadTransferUpdate.getStatus() == PayloadTransferUpdate.Status.SUCCESS) {
                    ...
                    SwishhLogger.d("SwishhSDK", "Payload Success ");
                    if (payload != null) {
                        if (payload.getType() == Payload.Type.FILE) {
                            SwishhLogger.d("SwishhSDK", "Payload File Success");
                            SwishhLogger.d("SwishhSDK", "Payload File Length " + payload.asFile().getSize());
                            SwishhLogger.d("SwishhSDK", "Payload File Bytes " + payload.asBytes());


                            File payloadFile = payload.asFile().asJavaFile();

                            if (payloadFile != null) {
                                SwishhLogger.d("SwishhSDK", "Payload Not null");
                            } else {
                                SwishhLogger.d("SwishhSDK", "Payload null");
                            }
                    ...
                    SwishhLogger.d("SwishhSDK", "Payload In transfer " + payloadTransferUpdate.getBytesTransferred() + " out of " + payloadTransferUpdate.getTotalBytes());
                }
            }
        }).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                SwishhLogger.d("SwishhSDK", "Accepted Connection Success0");
                userAcceptCallback.onConnectionAccepted(true, userId, 0);
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                SwishhLogger.d("SwishhSDK", "Fail to accept0 " + e.toString());
                if (e.getMessage() != null) {
                    int getCode = Integer.parseInt(e.getMessage().replaceAll("[^0-9]", ""));
    ...
    public void sendMessage(String userId, final byte[] message) {
        try {
            mConnectionsClient.sendPayload(userId, Payload.fromBytes(message)).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    SwishhLogger.d("SwishhSDK", "Message sent successfully");
                }
            });
        } catch (Exception e) {
            SwishhLogger.d("SwishhSDK", "Message sent failed");
        }
    }
    ...
    public void sendFile(String userId, File file) {
        SwishhLogger.e("SwishhSDK", "File to be sent to " + userId + " , file size " + file.length());
        try {
            mConnectionsClient.sendPayload(userId, Payload.fromFile(file)).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    SwishhLogger.e("SwishhSDK", "File sending failed" + e.getMessage() + " : " + e.getCause() + " : " + Arrays.toString(e.getStackTrace()));
                }
            }).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    SwishhLogger.d("SwishhSDK", "File sending");

                }
            });
        } catch (Exception e) {
            SwishhLogger.e("SwishhSDK", "File sent failed" + e.getMessage() + " : " + e.getCause() + " : " + Arrays.toString(e.getStackTrace()));
        }
    }
