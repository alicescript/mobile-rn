package tech.swishh.gobridge.bledriver;
...
public class PeerDevice {
    ...
    private BluetoothGattService mSwishhService;
...
//                            mConnectionLatch.countDown();
    ...
    private BluetoothGattService getSwishhService() {
        return mSwishhService;
    }

    private void setSwishhService(BluetoothGattService service) {
        mSwishhService = service;
    }
    ...
    // takeSwishhService get the Swishh service in the list of services
    private boolean takeSwishhService() {
        mLogger.v(TAG, String.format("takeSwishhService: called for device %s", mLogger.sensitiveObject(getMACAddress())));

        setSwishhService(getBluetoothGatt().getService(GattServer.SERVICE_UUID));

        if (getSwishhService() == null) {
            mLogger.e(TAG, String.format("Swishh service not found for device=%s", mLogger.sensitiveObject(getMACAddress())));
            return false;
        }

        mLogger.d(TAG, String.format("Swishh service found for device=%s", mLogger.sensitiveObject(getMACAddress())));
        return true;
    }
...
//    }
    ...
    // takeSwishhCharacteristics checks if the service has the two characteristics expected.
    private boolean takeSwishhCharacteristics() {
        mLogger.v(TAG, String.format("takeSwishhCharacteristic called for device=%s", mLogger.sensitiveObject(getMACAddress())));

        List<BluetoothGattCharacteristic> characteristics = getSwishhService().getCharacteristics();
...
//        mLogger.d(TAG, String.format("getRandomBytes: device=%s bytes=%s", mLogger.sensitiveObject(getMACAddress()), BleDriver.bytesToHex(arr)));
    ...
    // handshake identifies the swishh service and their characteristic, and exchange the peerID each other.
    private void handshake() {
        mLogger.d(TAG, "handshake: called");

        if (takeSwishhService()) {
            if (takeSwishhCharacteristics()) {
        ...
        mLogger.e(TAG, String.format("handshake error: failed to find swishh service: device=%s", mLogger.sensitiveObject(getMACAddress())));
        disconnect();
    }
